...
const Registation = () => {
  ...
  const handelonSubmit = (event) => {
    ...
    const ChackBox=event.target.chackbox.checked;
    console.log(FirstName, LastName, Email, Password ,ChackBox)
    //password valitation ....
    if(!ChackBox){
      setErrormasses('Please Checkbox Active')
      return;
    }
            ...
            <div className="card-body ">
              <form onSubmit={handelonSubmit}>
                <fieldset className="fieldset relative">
                  ...
                  <input type={chackPassword ? "text" : "password"} name='password' className="input" placeholder="Password" />
                <h1
                  onClick={() => (setChackpassword(!chackPassword))}
                className=' text-xl absolute right-9 bottom-19'>
                {chackPassword ? <FaEyeSlash></FaEyeSlash> : <FaEye></FaEye >}
                </h1>
                ...
                name='chackbox'
              />
              <h1 className='font-bold'>Accept Privacy Policy</h1>
              </div>
              </form>
